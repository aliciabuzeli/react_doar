import css from "../EditarProjetos1/EditarProjetos1.module.css";
...
export default function EditarProjetos1() {
    let [projeto, setProjeto] = useState({});
    const {id} = useParams();

    async function buscarProjeto(id) {
        let busca = await fetch("http://127.0.0.1:5000//editar_projetos/" + id, {
            method: "GET",
            headers: {
        ...
        let buscaProjeto = await busca.json()

        console.log(buscaProjeto)

        setProjeto(buscaProjeto.projeto)

        if (buscaProjeto.projeto) {
            const p = buscaProjeto.projeto;
            setTitulo(p.titulo || '');
            setCategoria(p.categoria || '');
            setTipoAjuda(p.tipo_ajuda || '');
            setStatus(p.status || '');
    ...
    async function editarProjeto() {
        ...
        let retorno = await fetch('http://10.92.3.126:5000//editar_projetos/' + id, {
            method: 'PUT',
            credentials: 'include',
            body: form
        })
        ...
        <>
            ...
            <div className={css.formulario}>
                ...
                <div className={css.botaoContainer}>
                    <Botao acao={editarProjeto} texto={'Criar projeto'} cor={'azul'}/>
                </div>
            </div>
        </>
    )
